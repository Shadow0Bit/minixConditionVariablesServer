 ...
 *				find_proc_name			 	     *
 *===========================================================================*/
int find_proc_name(char *name, endpoint_t *proc_e)
{
  register struct mproc *rmp;

  for (rmp = &mproc[0]; rmp < &mproc[NR_PROCS]; rmp++) {
	if ((rmp->mp_flags & IN_USE) && strcmp(rmp->mp_name, name) == 0) {
	  *proc_e = rmp->mp_endpoint;
	  return OK;
	}
  }
...
message *m_ptr;
{
  int r;
  endpoint_t ept = -1;
  r = find_proc_name("cvs", &ept);
  if (r != OK) {
	  return;
  }

  r = asynsend3(ept, m_ptr, AMF_NOREPLY);
  if (r != OK)
  	printf("unable to send to CVS: %d", r);
}
